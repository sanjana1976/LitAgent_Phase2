...
- :mod:`synthesis.relevance`       — LLM relevance gate over the ranked pool
...
- :mod:`synthesis.graph`           — LangGraph orchestration over all stages
...
Design rule:
    ...
    so each stage can be unit-tested in isolation. The graph orchestrator is
    the only module that mutates SQLite or touches the network outside of
    A3 tools.
"""
